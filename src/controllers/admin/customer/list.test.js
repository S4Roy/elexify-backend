...
describe('customer directory', () => {
  it('combines import/search filters and enriches only the requested page with linked order activity', async () => {
    ...
    const devices = vi.spyOn(DeviceToken, 'aggregate').mockResolvedValue([{ _id: id, count: 2 }]);
    ...
    expect(devices).toHaveBeenCalledWith([
      { $match: { user_id: { $in: [id] }, environment: 'staging', project_id: 'test-project', is_active: true } },
  ...
  it('avoids querying orders for an empty customer page', async () => {
    ...
    const devices = vi.spyOn(DeviceToken, 'aggregate');
