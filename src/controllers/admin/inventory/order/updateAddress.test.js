...
describe('order address eligibility', () => {
  ...
  it.each([
    ...
    { invoice: { generated: true } }, { package_count: 1 }, { awb: 'AWB' },
    { shiprocket_order_id: 'shipment' }, { shipped_at: new Date() }, { refund: { status: 'processing' } },
...
describe('order address updates', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(Package, 'exists').mockReturnValue({ session: async () => null });
  ...
  it.each(['package', 'invoice'])('blocks existing %s even when the order flags are stale', async kind => {
    (kind === 'package' ? Package : Invoice).exists.mockReturnValue({ session: async () => ({ _id: oid() }) });
    await updateAddress(req, res, next);
    expect(next.mock.calls[0][0].statusCode).toBe(409);
    expect(Address.create).not.toHaveBeenCalled();
  });
