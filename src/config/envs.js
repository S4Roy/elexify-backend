...
export const envs = {
  ...
  FAST2SMS: {
    ...
    // Per-template sender_id can still override this (SmsTemplate.sender_id)
    // — this is only the fallback when a template doesn't set its own.
    sender_id: process.env.FAST2SMS_SENDER_ID || "ELXFY",
    route: "dlt",
    // DLT-approved Fast2SMS message_id + variable order per event now live
    // in the DB (models/SmsTemplate.js, admin-editable), not here — see
    // services/smsTemplate/getTemplate.js. This used to be a hardcoded
    // per-event env var map; moved to the DB so an admin can add/update a
    // DLT-approved template (message_id, copy, variable order) without a
    // redeploy, the same way EmailTemplate already works.
  },
  whatsapp: {
