...
export const findContact = async (connection, payload) => {
  const byIdentity = await findExact(connection, "contacts", { contact_name: payload.contact_name, filter_by: "Status.All" },
    candidate => candidate.contact_type === "customer" && candidate.contact_name === payload.contact_name);
  ...
  if (mobile) {
    ...
    if (match) return match;
    return findExact(connection, "contacts", { filter_by: "Status.All" }, matchesMobile);
  }
  return null;
};
...
export const syncContact = async (connection, userId, order = null) => {
  ...
  const billing = order?.billing_address_snapshot || address;
  const shipping = order?.shipping_address_snapshot || billing;
